		...
		<div class="container">
			<div class="hero-banner">
				<div class="row">
					...
					<div class="main-btn">
						<a class="buy-now link-btn" href="#">BUY NOW AND SAVE UP TO 50%</a>
						<a class="free-day link-btn" href="#">FREE 14 DAY TRIAL</a>
					</div>
				</div>
			</div>
		</div>
						...
						<div class="list">
							...
							@foreach($plans as $plan)
								...
								<div class="list-left">
									<div class="list-head">
										<p>{{$plan->plan_name}}</p>
										<span>CES {{number_format($p_price[0]->price)}}  <del class="discount">{{number_format($p_price[0]->price*2)}}</del> <sub>Per {{$p_price[0]->duration_name}} </sub></span>
									...
									<div class="list-table">
										...
										@foreach($p_price as $price)
											<tr>
												<td>{{ $price->duration_name.' ('.$price->duration_value.'M)'}}</td>
												<td class="red">CES {{ number_format( $price->price)}}</td>
											 </tr>
										@endforeach


										</table>
											<a class="<?php  if($flag==1){echo"left"; $flag=2;}else{ echo"right"; $flag=1;}?>-btn list-btn" data-toggle="modal" data-target="#myModal" href="javascript:void(0)">Buy Now</a>
